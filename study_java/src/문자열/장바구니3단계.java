				...
				if (sel == 1) {
					...
					if (num == 1) {
						...
						while (true) {
							...
							if(cnt==0) {
							System.err.println("존재하지 않는 아이템 입니다.");
							continue;
							}
							System.out.printf("%s 몇개 삭제할지 입력하세요 ", input);
							int del = scan.nextInt();

							if (del > cnt) {
								System.err.println("장바구니에 들어있는 갯수 초과");
								continue;
							}
							if (del < 0) {
								System.err.println("0보다 큰 정수를 입력하세요");
								continue;
							}
							for (int i = 0; i < del; i++) {
								for (int k = 0; k < count; k++) {
									if (input.equals(cartList[k][1]) && idList[log].equals(cartList[k][0])) {
